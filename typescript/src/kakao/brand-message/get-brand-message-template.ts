...
export class GetBrandMessageTemplate extends BaseScenario {
  description = '[카카오] 브랜드메시지 템플릿 상세 조회'

  async execute() {
    try {
      const response = await this.sendon.kakao.getBrandMessageTemplate(
        KKO_SEND_PROFILE_ID,
        KKO_BRAND_MESSAGE_TEMPLATE_CODE,
      )
